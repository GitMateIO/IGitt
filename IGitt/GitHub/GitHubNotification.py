...
class GitHubNotification(GitHubMixin, Notification):
    ...
    def subject_type(self) -> Subject:
        ...
        return {
            ...
            'Issue': Subject.ISSUE,
            'PullRequest': Subject.MERGE_REQUEST
        }.get(self.data['subject']['type'])
    ...
    def subject(self):
        ...
        subject = {
            Subject.COMMIT: GitHubCommit,
            Subject.ISSUE: GitHubIssue,
            Subject.MERGE_REQUEST: GitHubMergeRequest
        }.get(self.subject_type)
        id = subject_data['url'].strip().split('/')[-1]
        return subject.from_data(subject_data, self._token,
                                 self.repository.full_name, id)
